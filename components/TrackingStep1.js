...
export default function Step1Page({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.sender_name || !formData.sample_code || !formData.quantity) {
        throw new Error('Nama pengirim, kode sample, dan jumlah harus diisi');
      }
      ...
      Object.keys(formData).forEach(key => {
        if (key !== 'photo_url' || (key === 'photo_url' && formData[key])) {
                ...
                {!previewUrl ? (
                  <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                    <div className="space-y-1 text-center">
                      <Upload className="mx-auto h-12 w-12 text-gray-400" />
                      <div className="flex text-sm text-gray-600">
                        <label htmlFor="document_url" className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none">
                          <span>Upload file</span>
                          <input id="document_url" name="document_url" type="file" accept="uploads/*" onChange={handleFileChange} className="sr-only" />
                        </label>
                        <p className="pl-1">atau drag and drop</p>
                      </div>
