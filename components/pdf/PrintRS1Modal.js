      ...
      axios.get(`/api/print-rs1?sampleOrderNo=${sampleOrderNo}`)
        .then(response => {
          ...
          const generateQR = async () => {
            const qrUrl = await generateSignedQRCode(response.data, response.data.signed_image_path);
            setQrCodeUrl(qrUrl);
          };
...
const [qrCodeUrl, setQrCodeUrl] = useState(null);
  ...
  useEffect(() => {
    const fetchQRCode = async () => {
      if (data) {
        try {
          const qrUrl = await generateSignedQRCode(data, data.signed_image_path);
          setQrCodeUrl(qrUrl);
        } catch (error) {
            ...
            <View style={[styles.table, { marginTop: 5 }]}>
              <View style={styles.senderOrderRow}>
                ...
                <View style={styles.orderCell}>
                  ...
                  <View style={{ flexDirection: 'row', marginTop: 5 }}>
                    <Text style={{ width: 130 }}>Email</Text>
            ...
            <View style={styles.retentionRow}>
              <View style={styles.qrCodeCell}>
                ...
                {qrCodeUrl && (
                  <Image 
                    src={qrCodeUrl} 
                    style={styles.qrCode}  // Pastikan Anda punya style ini di styles
                  />
                )}
