...
export default function Step2({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      data.append('received_quantity', formData.received_quantity);
      data.append('received_date', new Date().toISOString());
      data.append('received_by', formData.receiver_name); // Simpan juga sebagai received_by
      
      // Append file if it exists
      if (formData.received_photo_url) {
        data.append('file-upload', formData.received_photo_url);
      }
      
      // Debug logs
      console.log(`Mengirim request PUT ke /api/tracking-samples/step2/${id}`);
      
      // Send data to API dengan method PUT
          ...
          <div className="p-4 bg-blue-50 border-l-4 border-blue-500">
            ...
            <div className="grid grid-cols-2 gap-4 mt-2 text-sm">
              ...
              <div>
                ...
                <p className="font-semibold">{sampleData.driver_name || '-'}</p>
              </div>
              <div>
                <p className="text-gray-600">Plat Number:</p>
                <p className="font-semibold">{sampleData.plate_number || '-'}</p>
              </div>
            </div>
          </div>
        )}
