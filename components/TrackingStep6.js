    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (sampleData.coa_issued_date) {
          setCurrentTab('final');
        } else if ((sampleData.coa_review_status === 'approved' || sampleData.coa_review_status === 'rejected') || 
                  (currentUser.role === 'coa_reviewer')) {
          ...
          <form onSubmit={handleSubmit} className="p-6">
            ...
            {currentTab === 'review' && (
              <div className="space-y-6">
                ...
                <div>
                  <label htmlFor="coa_reviewer" className="block text-sm font-medium text-gray-700 mb-1">
                    Nama Reviewer
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label htmlFor="coa_review_status" className="block text-sm font-medium text-gray-700 mb-1">
                    Status Review
                  </label>
                  <select
                    id="coa_review_status"
                    name="coa_review_status"
                    value={formData.coa_review_status}
                    onChange={handleChange}
                    required
                    disabled={!canSubmitReview() || !formData.coa_draft_url}
                    className={`w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 ${(!canSubmitReview() || !formData.coa_draft_url) ? 'bg-gray-100' : ''}`}
                ...
                {formData.coa_review_status !== 'approved' && (
                  <div className="bg-yellow-50 p-4 rounded-md flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-yellow-600 mt-1">
                        Draft COA harus disetujui oleh coa_reviewer terlebih dahulu sebelum dapat menerbitkan COA final.
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center">
                    ...
                    <div className="flex items-center space-x-2">
                      <label 
                        htmlFor="final_file" 
                        className={`px-4 py-2 border border-gray-300 rounded-md cursor-pointer flex items-center space-x-2 ${
                          formData.coa_review_status !== 'approved' 
                            ? 'bg-gray-100 text-gray-400 cursor-not-allowed' 
            ...
            <div className="mt-8 flex justify-end space-x-3">
              ...
              <button
                ...
                disabled={
                  submitting || 
                  (currentTab === 'final' && formData.coa_review_status !== 'approved') ||
                  (currentTab === 'draft' && !canSubmitDraft()) ||
                  (currentTab === 'review' && (!canSubmitReview() || !formData.coa_draft_url))
                }
                className={`px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white flex items-center space-x-2 ${
                  submitting || 
                  (currentTab === 'final' && formData.coa_review_status !== 'approved') ||
                  (currentTab === 'draft' && !canSubmitDraft()) ||
                  (currentTab === 'review' && (!canSubmitReview() || !formData.coa_draft_url))
