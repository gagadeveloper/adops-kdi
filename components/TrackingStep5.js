...
export default function Step5({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let documentUrl = formData.roa_document_url;
      ...
      const requestData = {
        ...formData,
        roa_document_url: documentUrl,
        roa_created_date: sampleData.roa_created_date || new Date().toISOString()
      };
      
      // If status is issued, add issued date
      if (formData.roa_status === 'issued') {
        requestData.roa_issued_date = new Date().toISOString();
      }
