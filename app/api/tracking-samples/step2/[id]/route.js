// File: app/api/tracking-samples/step2/[id]/route.js
...
export async function PUT(request, { params }) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ message: 'ID sample tidak ditemukan' }, { status: 400 });
    }
    
    // Parse form data dengan file upload
    const formData = await request.formData();
    const photoFile = formData.get("file-upload"); // Sesuaikan dengan nama field di form
